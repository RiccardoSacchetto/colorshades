...
function ColorBanner({ backgroundColor, isMobile, setToast }) {
  ...
  function rgbToHex(rgb) {
    const r = `${Math.floor(rgb.r / 16).toString(16)}${(rgb.r % 16).toString(
      16
    )}`
    const g = `${Math.floor(rgb.g / 16).toString(16)}${(rgb.g % 16).toString(
      16
    )}`
    const b = `${Math.floor(rgb.b / 16).toString(16)}${(rgb.b % 16).toString(
      16
    )}`
  ...
  return (
    ...
    >
      <div
        className="color-banner-details"
        onClick={isMobile ? copyText : null}
      >
        <p className="hex-color" style={textStyle}>
          #{hexColor}
        </p>
